...
class SKA2025parser:
	...
	def entryRawText_to_entry(self, raw_entry: EntryRawText) -> Entry:
		header_match = self.ENTRY_REGEX.match(raw_entry.header)
		if not header_match:
			raise ValueError(f"Header does not match expected format: {raw_entry.header}")
		date, kind, amount = header_match.groups()

		subject_match = self.SUBJECT_REGEX.search(raw_entry.subject)
		if subject_match:
	...
	def extract_raw_entries_from_page_text(self, text: str) -> List[EntryRawText]:
		...
		for line in lines:
			if self.SIGNATURE_BLOCK_REGEX.match(line) or self.FINAL_BALANCE_REGEX.match(line) and not is_reading_signature:
				is_reading_signature = True
				continue
			if self.ENTRY_REGEX.match(line):
